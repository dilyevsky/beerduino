...
PROTO_OBS      = $(OBJDIR)/beerduino.pb.o
...
# General arguments
USER_CPPFLAGS = $(patsubst %,-D%,$(ARDUINO_CPPFLAGS))
...
USER_LIB_CC_SRCS    = $(wildcard $(patsubst %,%/*.cc,$(USER_LIBS)))
...
USER_LIB_OBJS = $(patsubst $(USER_LIB_PATH)/%.cpp,$(OBJDIR)/libs/%.o,$(USER_LIB_CPP_SRCS)) \
		$(patsubst $(USER_LIB_PATH)/%.cc,$(OBJDIR)/libs/%.o,$(USER_LIB_CC_SRCS)) \
		$(patsubst $(USER_LIB_PATH)/%.c,$(OBJDIR)/libs/%.o,$(USER_LIB_C_SRCS))

CPPFLAGS      = -mmcu=$(MCU) -DF_CPU=$(F_CPU) -DARDUINO=$(ARDUINO_VERSION) $(USER_CPPFLAGS) \
...
$(OBJDIR)/libs/%.o: $(USER_LIB_PATH)/%.cc
	mkdir -p $(dir $@)
	$(CC) -c $(CPPFLAGS) $(CFLAGS) $< -o $@
...
# Protobuf rules.
$(OBJDIR)/%.pb.o: $(OBJDIR)/%.pb.c
	$(CC) -c $(CPPFLAGS) $(CFLAGS) $< -o $@

$(OBJDIR)/%.pb.c: ../proto/beerduino.proto
	protoc -I/usr/local/include -I../nanopb/generator -I../proto -o$(@:.c=) $<
	protoc --python_out=../nanopb/generator/ -I/usr/local/include -I../nanopb/generator ../nanopb/generator/nanopb.proto
	python ../nanopb/generator/nanopb_generator.py $(@:.c=)
...
$(TARGET_ELF): 	$(PROTO_OBS) $(LOCAL_OBJS) $(CORE_LIB) $(OTHER_OBJS)
		$(CC) $(LDFLAGS) -o $@ $^ -lc -lm
...
clean:
		$(REMOVE) $(LOCAL_OBJS) $(CORE_OBJS) $(LIB_OBJS) $(CORE_LIB) $(TARGETS) $(DEPS) $(USER_LIB_OBJS)
